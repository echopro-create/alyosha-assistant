...
class MessageBubble(QFrame):
    ...
    def __init__(self, text: str, role: str = "user", parent=None):
        ...
        # Don't set graphics effect here to avoid conflict with shadow
    ...
    def resizeEvent(self, event):
        """Динамическое ограничение ширины пузырька (резиновость)"""
        super().resizeEvent(event)
        if self.parentWidget():
            # Max width is 85% of available space
            max_w = int(self.parentWidget().width() * 0.85)
    ...
    def animate_in(self):
        ...
        self.setMaximumHeight(16777215)  # Remove limit temporarily
        self._target_height = self.sizeHint().height()
        self.setMaximumHeight(0)
        
        # Height animation with overshoot (spring effect)
        self.height_anim = QPropertyAnimation(self, b"maximumHeight")
        self.height_anim.setDuration(400)
        self.height_anim.setStartValue(0)
        self.height_anim.setEndValue(self._target_height + 16)
        self.height_anim.setEasingCurve(QEasingCurve.Type.OutBack)  # Spring overshoot
        
        # CRITICAL: Reset maximumHeight after animation to allow dynamic resizing
        self.height_anim.finished.connect(lambda: self.setMaximumHeight(16777215))
...
class ChatWidget(QWidget):
    ...
    def add_message(self, text: str, role: str = "user"):
        ...
        # Scroll to bottom
        QTimer.singleShot(450, self._scroll_to_bottom)
    ...
    def load_messages(self, messages: list[dict], animate: bool = False):
        ...
        for msg in messages:
            ...
            self.messages_layout.insertWidget(self.messages_layout.count() - 1, bubble)
            if not animate:
                bubble.setMaximumHeight(16777215)  # Skip animation
            else:
                QTimer.singleShot(20, bubble.animate_in)
