    ...
    def _execute_tool(self, command: str) -> str:
        ...
        try:
            ...
            if tool_name == "calculate":
                ...
                SystemControl.type_text(args)
                return "Текст напечатан"
            elif tool_name == "status":
                return SystemControl.get_system_status()
            elif tool_name == "media":
                SystemControl.media_control(args.strip())
                return "Медиа команда выполнена"
            elif tool_name == "clipboard":
                # Expect "get" or "set <text>"
                if " " in args:
                    action, text = args.split(" ", 1)
                    return SystemControl.clipboard_control(action, text)
                else:
                    return SystemControl.clipboard_control(args.strip())
            elif tool_name == "window":
                SystemControl.window_control(args.strip())
                return "Команда окна выполнена"
